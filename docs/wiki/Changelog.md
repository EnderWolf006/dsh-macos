...
- 配套（dsh-launcher 仓）：mini-dialog 插件 client 声明修复——inject 改官方 ui-workspace 同款 9 包列表（原 `dsh-client-runtime` 已不存在，悬空边曾炸浏览器启动图致白屏）；汉化插件在 0.1.2-alpha 上原样可用
